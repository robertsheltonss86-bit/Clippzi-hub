...
// iconUrl paths are served by the clippzi web artifact from /clippzi/gifts/*
const PREMIUM_GIFTS: Seed[] = [
  { name: "Thor's Hammer", emoji: "🔨", price: "199.99", category: "legendary", rarity: "legendary", description: "Mjölnir crashes down with thunder & lightning — only the worthy can wield it", iconUrl: "/clippzi/gifts/thors-hammer.png" },
  { name: "Gorilla vs T-Rex", emoji: "🦍", price: "699.99", category: "legendary", rarity: "legendary", description: "A silverback gorilla charges and beats up a tyrannosaurus rex — full screen brawl", iconUrl: "/clippzi/gifts/gorilla-vs-trex.png" },
  ...
  { name: "King of Atlanta Crown", emoji: "👑", price: "499.99", category: "legendary", rarity: "legendary", description: "A diamond-encrusted crown rains gold across the stream", iconUrl: "/clippzi/gifts/king-crown.png" },
  ...
  { name: "Phoenix Rebirth", emoji: "🔥", price: "249.99", category: "legendary", rarity: "legendary", description: "A 3D phoenix bursts from ashes — full-screen fire animation", iconUrl: "/clippzi/gifts/phoenix-rebirth.png" },
  { name: "Dragon Strike", emoji: "🐉", price: "399.99", category: "legendary", rarity: "legendary", description: "An ancient dragon breathes fire across the entire stream", iconUrl: "/clippzi/gifts/dragon-strike.png" },
  ...
  { name: "Lamborghini Drop", emoji: "🏎️", price: "599.99", category: "legendary", rarity: "legendary", description: "A neon Lambo speeds across the screen with smoke trails", iconUrl: "/clippzi/gifts/lambo-drop.png" },
  ...
  { name: "Stack of Cash", emoji: "💰", price: "150.00", category: "legendary", rarity: "legendary", description: "Bands of $100 bills rain from the top of the screen", iconUrl: "/clippzi/gifts/stack-of-cash.png" },
  ...
  { name: "Treasure Chest", emoji: "🪙", price: "24.99", category: "epic", rarity: "epic", description: "Pirate's chest bursts open with gold coins", iconUrl: "/clippzi/gifts/treasure-chest.png" },
...
export async function seedPremiumGifts(): Promise<void> {
  try {
    ...
    for (const g of PREMIUM_GIFTS) {
      const [existing] = await db.select({ id: giftsTable.id, iconUrl: giftsTable.iconUrl }).from(giftsTable).where(eq(giftsTable.name, g.name)).limit(1);
      if (existing) {
        // Backfill iconUrl on existing rows that don't have one yet (or whose URL changed)
        if (g.iconUrl && existing.iconUrl !== g.iconUrl) {
          await db.update(giftsTable).set({ iconUrl: g.iconUrl }).where(eq(giftsTable.id, existing.id));
          updated++;
        }
        continue;
      }
    ...
    if (inserted > 0 || updated > 0) logger.info({ inserted, updated }, "Seeded premium gifts");
  } catch (err) {
